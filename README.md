# Windows用64bit版Emacs29.3のビルドを支援するツール

## 概要

MSYS2のダウンロードから64bit版Emacs29.3のビルドまでをサポートするツールです。主な特色は次のとおりです。

  * 既存の環境を汚すことなくEmacs29.3のビルドができます。
  * cmigemo-moduleを組み込みます。
  * Emacs29.3関連ファイルのみをまとめたZIPファイルを作成します。
...
ビルド完了後、デスクトップに`emacs-29.3-x86_64-ime-日付.zip`のような名前のファイルが作成されます。IMEパッチが適用されていないときには、ファイル名に`-ime`は付きません。
